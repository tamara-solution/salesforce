...
var tamaraServiceRefund = {
    ...
    initService: function(order, requestBody, trackingChange){
        ...
        const service = tamaraHelper.getService(tamaraHelper.SERVICE.PAYMENT.REFUND);

        if (!service) {
            throw new Error('Could not connect to ' + tamaraHelper.SERVICE.PAYMENT.REFUND);
        }
        ...
        if (!resultObject) {
            throw new Error('No correct response from ' + tamaraHelper.SERVICE.PAYMENT.REFUND);
        }
        tamaraHelper.refundOrder(order, trackingChange);
...
module.exports = tamaraServiceRefund;