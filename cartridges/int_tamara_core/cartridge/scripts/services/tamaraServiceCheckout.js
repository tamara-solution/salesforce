...
var tamaraServiceCheckout = {
  ...
  initCheckoutSessionObject: function (orderNumber, paymentInstrument) {
    ...
    if (!order) {
      throw Error(
        "tamaraRequestObject.initCheckoutSessionObject(): Can not get the order from orderNumber " +
          orderNumber
      );
    }
    ...
    if (!shippingAddress) {
      throw Error(
        "tamaraRequestObject.initCheckoutSessionObject(): Missing shippingAddress in orderNumber " +
          orderNumber
      );
    }
    ...
    if (!billingAddress) {
      throw Error(
        "tamaraRequestObject.initCheckoutSessionObject(): Missing billingAddress in orderNumber " +
          orderNumber
      );
    }
    ...
    const token = tamaraHelper.generateToken(
      JSON.stringify({
        orderNo: orderNumber,
    ...
    for (let i = 0; i < productLineItems.length; i++) {
      let lineItem = {
        ...
        sku:
          productLineItems[i].getManufacturerSKU() ||
          productLineItems[i].productID,
        image_url:
          productLineItems[i].product
            .getImage("small", 0)
            .getAbsURL()
            .toString() || "",
        item_url:
        ...
        discount_amount: {
          amount: Number(
            (
              productLineItems[i].basePrice.value *
                productLineItems[i].quantity.value -
              productLineItems[i].adjustedNetPrice.value
            ).toFixed(2)
          ),
          currency: currencyCode,
        },
    ...
    return {
      ...
      consumer: {
        ...
        phone_number:
          customer && customer.getPhoneMobile()
  ...
  initService: function (order, paymentInstrument, paymentProcessor) {
    const service = tamaraHelper.getService(
      tamaraHelper.SERVICE.CHECKOUT.SESSION
    );
    if (!service) {
      throw new Error(
        "Could not connect to " + tamaraHelper.SERVICE.CHECKOUT.SESSION
      );
    }

    service.setRequestMethod("POST");
    const requestObject = tamaraServiceCheckout.initCheckoutSessionObject(
      order.getOrderNo(),
      paymentInstrument
    );
    ...
    if (!callResult.object) {
      throw new Error(
        "No correct response from " + tamaraHelper.SERVICE.CHECKOUT.PAYMENTTYPES
      );
    }
    ...
    Transaction.wrap(function () {
      ...
      order.custom.tamaraFailureNotification =
        requestObject.merchant_url.failure;
      order.custom.tamaraSuccessNotification =
        requestObject.merchant_url.success;
