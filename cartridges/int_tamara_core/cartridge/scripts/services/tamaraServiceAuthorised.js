...
var tamaraServiceAuthorised = {
     ...
     */
    initService: function(order, paymentInstrument){
        var tamaraHelper = require('*/cartridge/scripts/util/tamaraHelper');

        const service = tamaraHelper.getService(tamaraHelper.SERVICE.ORDER.AUTHORISED);
        if (!service) {
            throw new Error('Could not connect to ' + tamaraHelper.SERVICE.ORDER.AUTHORISED);
        }
