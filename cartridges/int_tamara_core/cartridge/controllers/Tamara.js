...
function getTokenObject(req) {
  var token =
    req.querystring && req.querystring.token ? req.querystring.token : "";
  if (!token) {
    throw Error(
      'Tamara-Success controller requires "token" in parameter, but not found in the request.'
    );
  }
  ...
  if (!tokenObject || !tokenObject.orderNo || !tokenObject.paymentMethodID) {
    throw Error(
      'Tamara-Success controller requires "orderNo" & "paymentMethodID" in token object, but not found in the request.' +
        tamaraHelper.decryptToken(token)
    );
  }
...
server.get("Success", server.middleware.https, function (req, res, next) {
  try {
    ...
    if (!order) {
      throw new Error(
        "Tamara-Success: Not found order object from orderNo " +
          responseObject.orderNo
      );
    }
...
server.get("Failure", server.middleware.https, function (req, res, next) {
  try {
    ...
    if (!order) {
      throw new Error(
        "Tamara-Failure: Not found order object from orderNo " +
          responseObject.orderNo
      );
    }
    const paymentInstrument = order.getPaymentInstruments(
      responseObject.paymentMethodID
    );
    if (!paymentInstrument.length) {
      throw new Error(
        'Tamara-Failure: Not found paymentInstrument "' +
          responseObject.paymentMethodID +
          '" from orderNo ' +
          responseObject.orderNo
      );
...
server.get("Cancel", server.middleware.https, function (req, res, next) {
  try {
    ...
    if (!order) {
      throw new Error(
        "Tamara-Cancel: Not found order object from orderNo " +
          responseObject.orderNo
      );
    }
    const paymentInstrument = order.getPaymentInstruments(
    ...
    if (!paymentInstrument.length) {
      throw new Error(
        'Tamara-Cancel: Not found paymentInstrument "' +
          responseObject.paymentMethodID +
          '" from orderNo ' +
          responseObject.orderNo
      );
...
server.use("Notify", server.middleware.https, function (req, res, next) {
  try {
    ...
    if (!order || body.order_reference_id !== order.orderNo) {
      throw new Error(
        "Tamara-Notify: Not found related order object from orderNo " +
          responseObject.orderNo
      );
    }
    ...
    if (!paymentInstrument.length) {
      throw new Error(
        'Tamara-Notify: Not found paymentInstrument "' +
          responseObject.paymentMethodID +
          '" from orderNo ' +
          responseObject.orderNo
      );
    ...
    res.render("tamara/tamaraFailure");
  }
  res.render("tamara/tamaraNotify");
  next();
});
