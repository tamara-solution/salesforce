...
server.use('Listen', server.middleware.https, function (req, res, next) {
    try {
        ...
        const paymentInstruments = order.getPaymentInstruments().toArray().filter(function (instrument) {
            return [tamaraHelper.METHOD_PAY_BY_INSTALMENTS, tamaraHelper.METHOD_TAMARA_INSTALMENTS, tamaraHelper.METHOD_TAMARA_6_INSTALMENTS].indexOf(instrument.paymentMethod) !== -1;
        });
